...
class CrackPassword(QThread):
    ...
    def run(self):
        ...
        if self.dict_source == 0:
            ...
            producer = PasswordsProducer("producer", queue, self.digit_range, self.seed, self.batch_size)
        # 如果使用外置字典，读取文件并运行
        elif self.dict_source == 1:
            ReadDict.password_num = 0
            producer = ReadDict("producer", queue, self.dict_path, self.batch_size)
        producer.producing_password.connect(self.on_producing_password, type=Qt.DirectConnection)
        producer.producing_password_num.connect(self.on_producing_password_num, type=Qt.DirectConnection)
        producer.start()
        # 定义若干消费者并运行
        pool = Pool(processes=self.consumer_number)
        result = pool.apply_async(extract_function, args=(queue, self.zipfile_path, self.extract_path, emit_queue))
        while True:
